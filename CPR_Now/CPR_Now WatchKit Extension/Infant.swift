...
class Infant: WKInterfaceController {
    
    @IBOutlet weak var infantFace: WKInterfaceImage!
...
//        watchFace.startAnimating()
           ...
           */
            infantFace.setImageNamed("progress")
            infantFace.startAnimatingWithImages(in: NSMakeRange(0, 30), duration: 60, repeatCount: -1)
       }
        ...
        func animateImage(_ imageName: String, _ color: UIColor) -> [UIImage] {
            ...
            (0...100).forEach { (index) in
                if let img = UIImage(named: "\(imageName)\(index)") {
                    images.append(img.imageCover(with: color))
                }
            }
            return images
        }
...
extension UIImage {
    func imageCover(with maskColor: UIColor) -> UIImage {
...
extension UIColor {
    ...
    @objc class func colorType (hex:String) -> UIColor {
